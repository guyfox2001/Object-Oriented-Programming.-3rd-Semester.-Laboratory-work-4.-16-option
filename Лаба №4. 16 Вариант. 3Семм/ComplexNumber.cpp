...
{
	this->Real = rand() % 15 + 1;
	this->Imaginary—oefficient = rand() % 15 + 1;
	this->update_MathematicalNotation();
}
...
void ComplexNumber::update_MathematicalNotation()
{
	...
	if (this->Imaginary—oefficient < 0) this->MathematicalNotation += "- " + std::to_string(abs(this->Imaginary—oefficient)) + 'i';
	else this->MathematicalNotation += "+ " + std::to_string(this->Imaginary—oefficient) + 'i';
}
